...
function shoptimizer_loop_product_get_rating_html(string $html, float $rating, int $count): string {
    ...
    return sprintf(
        '<div class="shoptimizer_ratingContainer" aria-label="%s">
            <div class="star-rating" role="img" aria-label="%s">%s</div>
            <div class="shoptimizer_ratingCount" aria-label="%s">(%d)</div>
        </div>',
        sprintf(esc_attr__('Product rating', 'shoptimizer')),
        sprintf(esc_attr__('Rated %s out of 5', 'shoptimizer'), number_format_i18n($rating, 2)),
        wc_get_star_rating_html($rating, $count),
        esc_attr__('Total number of reviews', 'shoptimizer'),
        $count
    );
}
